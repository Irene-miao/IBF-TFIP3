package sg.edu.nus.iss.day30.repository;
...
import static sg.edu.nus.iss.day30.repository.Queries.*;
...
public class RSVPRepository {
    ...
    public List<AggregrationRSVP> aggregateByFoodType(){
        ...
        /*db.rsvp.aggregate([  { $project: { name:1, food_type:1 }},
         {$group : {_id: "$food_type",  name: {$push : "$name"} ,
          count: {$sum :1}}}, { $sort: { count: -1}}]); */
        ProjectionOperation pop = Aggregation.project("name", "food_type", "_id");
        GroupOperation gOp = Aggregation.group("food_type")
                .push("name")
                .as("name")
                .count().as("count");
        ...
        AggregationResults<Document> aa = mongoTemplate.aggregate(pipeline, "rsvp",
                         Document.class);
        List<AggregrationRSVP> lstArrgFt = new LinkedList<>();
        Iterator<Document> id = aa.iterator();
        while(id.hasNext()){
            Document d1 = id.next();
            lstArrgFt.add(AggregrationRSVP.create(d1));
        }
        return lstArrgFt;
    }
